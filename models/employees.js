...
function Employees() {
    //region 員工
    //列出20個員工
    this.getEmployees = function(req, res) {
        var page = (typeof req.params.page != 'undefined' && req.params.page) ? req.params.page : 0;
        connection.acquire(function(err, con) {
            var query = con.query('select * from employee limit ?,?;', [parseInt(page) * 20, (parseInt(page) + 1) * 20], function(err, result) {
                con.release();
                res.json(result);
            });
    ...
    this.setEmployeeCreate = function(req, res) {
        ...
        connection.acquire(function(err, con) {
            var query = con.query('insert into employee set ?;', Employee, function(err, result) {
                if (err) {
                    console.log(err);
                    res.send("false");
                } else {
    ...
    this.setEmployeeDelete = function(req, res) {
        ...
        connection.acquire(function(err, con) {
            var query = con.query('delete from employee where _sn=?;', [Department._sn], function(err, result) {
                if (err) {
                    console.log(err);
                    res.send("false");
                } else {
    ...
    this.setEmployeeUpdate = function(req, res) {
        ...
        connection.acquire(function(err, con) {
            var query = con.query('update  employee set ? where _sn=?;', [Employee, sn], function(err, result) {
                if (err) {
                    console.log(err);
                    res.send("false");
                } else {
